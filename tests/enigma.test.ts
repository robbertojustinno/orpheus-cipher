...
describe('ORPHEUS enigma engine',()=>{
 ...
 it('production validator has no local canonical validation',async()=>expect((await new ProductionValidator().validate('box-01','test',{definition,state:available(),development:false})).feedbackCode).toBe('VALIDATOR_UNAVAILABLE'))
 ...
 it('unlocks the next hint without attempt gates',()=>expect(getNextHint(definition,available().enigmas['box-01'])).toMatchObject({allowed:true,hintId:'box-01-hint-01'}))
 ...
 it('calculates weighted progress across the 12-box campaign',()=>{const state=available();state.enigmas['box-01'].progress=100;expect(calculateOverallProgress(enigmaDefinitions,state)).toBe(8)})
