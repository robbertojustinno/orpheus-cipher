...
export type AuditEventType = 'COMMAND_EXECUTED'|'ENIGMA_OPENED'|'ENIGMA_ATTEMPTED'|'HINT_UNLOCKED'|'ENIGMA_SOLVED'|'UNLOCK_APPLIED'|'DOSSIER_ACCESSED'|'SECRET_COMMAND_DISCOVERED'|'MISSION_UPDATED'|'SEARCH_STARTED'|'SEARCH_COMPLETED'|'SEARCH_CANCELLED'|'RESULT_OPENED'|'REPORT_CREATED'|'ENTITY_CREATED'|'RELATIONSHIP_CREATED'|'RELATIONSHIP_CONFIRMED'|'RELATIONSHIP_REJECTED'|'HYPOTHESIS_CREATED'|'EVIDENCE_VIEWED'|'INVESTIGATION_DELETED'
...
export interface PersistedNarrativeState { version:number; narrativeState:NarrativeState; firstDetectionCompleted:boolean; missionStarted:boolean; founderId:number; founderTotal:number; enigmas:Record<string,PersistedEnigmaState>; unlockedFiles:string[]; unlockedMessages:string[]; unlockedDossiers:string[]; unlockedMapNodes:string[]; restoredFragments:string[]; discoveredCommands:string[]; auditEvents:AuditEvent[]; currentMissionId?:string; campaignStartedAt?:string; campaignCompleted:boolean; completionDate?:string; secretDiscoveries:string[]; unlockedBadges:string[]; lastSessionAt?:string }
